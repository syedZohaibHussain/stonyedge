          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                ...
                <button class="btn" style="float: right;" onclick="location.href='addCategory'"><i class="fa fa-plus"></i></button>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                @if (session('msg'))
                ...
                <table id="example1" class="table table-bordered table-striped text-center">
                  ...
                  <tr>
                    <td>{{$cat->cat_id}}</td>
                    <td>
                        @if($cat->image_url != null)<img src="{{url($cat->image_url)}}" height="50px" width="50px" />@else
                        no image
                        @endif</td>
                    <td>{{$cat->name}}</td>
                    <td>@if($cat->parent_id == 0)
                    Self parent
                    @else
                    @foreach($categories as $cats)
                    @if($cat->parent_id == $cats->cat_id)
                    {{$cats->name}}
                    @endif
                    @endforeach
                    @endif</td>
                    <td>
                        ...
                        <form action="{{route('category.index')}}" method="get">
                            @csrf
                            <input type="hidden" value="{{$cat->cat_id}}" name="id"/>
                            <button type="submit" class="badge p-2 bg-primary">{{$myVars}}</button>
                        </form>
                    ...
                    <button type="button" class="btn  btn-primary dropdown-toggle" data-toggle="collapse"data-target="#supplier{{$cat->cat_id}}">
                      <!-- <i class="fas fa-wrench"></i> -->
                      Action
                    </button>
                    <div class="dropdown-menu dropdown-menu-right collapse" role="menu" id="supplier{{$cat->cat_id}}">
                      <!--<a data-toggle="modal" href="#myView" data-target="#myView" class="dropdown-item"><i class="fas fa-binoculars mr-1"></i>View</a>-->
                      <a data-toggle="modal" href="#myEdit" data-target="#myEdit{{$cat->cat_id}}" class="dropdown-item"><i class="fas fa-edit mr-1"></i>Edit</a>
                      <a data-toggle="modal" href="#myDelete{{$cat->cat_id}}" data-target="#myDelete{{$cat->cat_id}}"class="dropdown-item"><i class="fas fa-trash mr-1"></i>Delete</a>
...
@foreach($category as $cat)
<!-- Modal Delete-->
<div class="modal fade" id="myDelete{{$cat->cat_id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<!-- Modal Edit-->
<div class="modal fade" id="myEdit{{$cat->cat_id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <form action="{{route('category.update',$cat->cat_id)}}" method="post" enctype="multipart/form-data">
          @csrf
          @method('PUT')
      ...
      <div class="modal-body">
       <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="cat_name" placeholder="Enter Category Name" required="required" value="{{$cat->name}}">
                  </div>
                  <div class="form-group">
                      <label>Parent Category<span style="color: red;"> *</span></label>
                      <select  class="form-control" id='selUser' name="parent_id" required="required">
                        <option value=""required disabled selected>Select Parent Category</option>
                        <option value="0"{{ 0 == $cat->parent_id ? 'selected="selected"' : '' }}>Self Parent</option>
                             @foreach($categories as $category)
                        <option value="{{$category->cat_id}}"{{ $category->cat_id == $cat->parent_id ? 'selected="selected"' : '' }}>{{$category->name}} || {{$category->slug}}</option>
                        @endforeach                 
                      </select>
                    </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="cat_description" placeholder="Enter Description" value="{{$cat->description}}">
                  </div>
                  <!-- <div class="form-group">
                   ...
                   <div class="form-group">
                    ...
                    <img src="{{url($cat->image_url)}}" height="100px" width="100px" />
                    <label>Image</label>
                    <input type="file" class="form-control" name="image_url">
                    <input type="hidden" class="form-control" name="old_image_url" value="{{$cat->image_url}}">
                    @else
                    <label>Image</label>
