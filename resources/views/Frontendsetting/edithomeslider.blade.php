        ...
        <div class="row">
          ...
          <div class="col-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
          <form action="{{route('homeslider.update',$details->id)}}" method="post" enctype="multipart/form-data" id="Supplierform">    
          @csrf
          @method('PUT')
                <div class="card-body">  
                  <div class="form-group">
                    <label>Section Name<span style="color: red;"> *</span></label>
                    <input type="text" class="form-control" name="section_name" value="{{$details->name}}"  required="required" >
                  </div>
                  
                    <div class="form-group">
                      <label>Sort Number<span style="color: red;"> *</span></label>
                      <input type="number" class="form-control" name="sort_no" placeholder="Enter Sort Number" required="required" value="{{$details->sort}}">
                      
                    </div>

                    <div class="form-group">
                      <label>Description</label>
                      <input type="text" class="form-control" name="description" placeholder="Enter Description" required="required" value="{{$details->description}}">
                      
                    </div>

                    <div class="form-group">
                      <label>Layout Section<span style="color: red;"> *</span></label>
                      <select  class="form-control" id='selUser' name="layout_id" required="required">
                        <option value=""required disabled selected>- Select Layout -</option>
                        @foreach($sections as $section)
                         <option value="{{$section->id}}" {{$details->section_layout_id == $section->id ? 'selected="selected"': ''}}>{{$section->name}}</option>
                         @endforeach
                                                                                
                      </select>
                    </div>
                    <div class="form-group">
                      <label>Product Section Slider<span style="color: red;"> *</span></label>
                      <select  class="form-control" id='selUser1' name="product_slider_id" required="required">
                        <option value=""required disabled selected>- Select Product -</option>
                         @foreach($sliders as $slider)
                         <option value="{{$slider->id}}" {{ $details->product_sliders_id == $slider->id? 'selected="selected"': ''}}>{{$slider->name}}</option>
                         @endforeach                      
                      </select>
                    </div>
                    <div class="form-group">
                          ...
                          <div class="row">
                          @if($details->main_image != null)
                         
                        <div class="col-md-2 pb-3 img">
                                        <img src="{{url($details->main_image)}}" width="150" height="100" alt="" style="border: 1px; border-style: solid;">
                          ...
                          <input type="file" class="form-control col-md-6" name="main_image"/>
                          <input type="hidden" name="old_main_image" value="{{$details->main_image}}" />
                    </div>
                    <div class="form-group">
                      <label>Main Section URL</label>
                        <input type="url" class="form-control col-md-6" name="main_url" placeholder="http://google.com" value="{{ $details->main_url}}">
                      
                    </div>
                    <div class="form-group">
                      <div class="row">
                        <div class="col-6">
                          <label>Enable Section Color</label>
                          <input type="text" class="form-control col-4" name="color_status" value="{{ $details->color_status}}"/>
                        </div>
                        <div class="col-6">
                          <label>Backg-round Color</label>
                          <input type="color" class="form-control col-4" name="bg_color" value="{{ $details->bg_color}}"/>
                        </div>
                      </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-2 pb-3 img text-center">
                                        <img src="{{url($image->image_url)}}" width="150" height="100" alt="" style="border: 1px; border-style: solid;">
                                        <div align="center" style="position: absolute;right: 8px;top: 0;"class="deletefile">
                                            <button type="button" data-id="{{$image->id}}"class="btn deletefiles removeimg
 btn-danger  btn-sm">x</button>
                                        
                                        </div>
                                        <div class="py-4">
                                          <input type="hidden" name="ids[]" value="{{$image->id}}">
                                        <input type="URL" class="form-control" name="image_url[]" placeholder="http://google.com"/ value="{{$image->url}}">
                                        <br>
                                        <input type="text" class="form-control" name="image_name[]" placeholder="Name" value="{{$image->name}}"/>
                                      </div>
                                    </div>
                    ...
                    <div class="row">
                    <div class="col-6">
                      <input type="file" class="form-control" name="section_image[]" multiple="" />
                    </div>
                      <div class="col-6">
                      <input type="hidden" class="form-control" name="alt_name" value="{{$details->alt_name}}" >
                      </div>
                    </div>
                      </div>
...
<script>
  $(document).on("click",'.removeimg',function(){
    ...
    var markup = '<input type="hidden" class="form-control" name="imgID[]" value="'+productId+'">';
